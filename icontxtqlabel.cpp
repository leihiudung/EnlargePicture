...
IconTxtQLabel::IconTxtQLabel(QWidget *parent, Qt::WindowFlags f)
    : QLabel{parent, f}
{
    setIndent(0);
}

void IconTxtQLabel::mousePressEvent(QMouseEvent* event)
{
    Q_UNUSED(event)
    emit clicked();
}

void IconTxtQLabel::paintEvent(QPaintEvent *event)
{
    //QElapsedTimer timer;
    //timer.start();
...
float IconTxtQLabel::getWidgetMaximumFontSize(QWidget *widget, QString text)
{
    ...
    qDebug() << currentWidth << "and" << currentHeight << ":" << currentSize << ":step" << step;
    return lastTestedSize;
}

void IconTxtQLabel::setTextColor(QColor color){
...
QColor IconTxtQLabel::getTextColor(){
    return textColor;
}


void IconTxtQLabel::setTextAndColor(const QString &text, QColor color){
    setTextColor(color);
    setText(text);
}

/* Do not give any size hint as it it changes during paintEvent */
QSize IconTxtQLabel::minimumSizeHint() const
{
    return QWidget::minimumSizeHint();
}

/* Do not give any size hint as it it changes during paintEvent */
QSize IconTxtQLabel::sizeHint() const
{
